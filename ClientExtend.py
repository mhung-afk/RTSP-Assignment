...
import socket, threading, sys, traceback, os, glob
...
class ClientExtend:
	...
	def exitClient(self):
		...
		if self.state == self.READY or self.state == self.PLAYING:
			...
			# delete cache
			for f in glob.glob("cache*.jpg"):
				os.remove(f)
			sleep(0.1)
			# statistic about this section
			if self.frameNbr > 0:
				if self.frameNbr > self.countLostFrame:
					print("Video data average rate: ", self.sumOfVidDataRate/(self.frameNbr - self.countLostFrame), " bps")		
		...
		while True:
			try:
				...
				if self.frameNbr < seqNumPacket:
					...
					self.sumOfVidDataRate += sys.getsizeof(rtpPacket.getPayload())/(time() - rtpPacket.timestamp())
